...
import NeighborHoodCollection from '@/components/NeighborHoodCollection';
...
const SearchNeighborHood = ({ className }: Props) => {
  const router = useRouter();
  const { city, neighborhood, setNeighborhood } = useLocationContext();
  const [neighborhoods, setNeighborhoods] = useState<GeoJSON.Feature[]>([]);
  const [open, setOpen] = useState(false);
  const [input, setInput] = useState('');
  let deleteTimeout: NodeJS.Timeout;

  useEffect(() => {
    if (!city?.properties) router.push('/request-office/0');
  }, [city]);
  ...
  const doneTyping = (value: string) => {
    if (value !== '') {
      fetch(
        `/api/geoapify?text=${value}&place=${city.properties?.place_id}&type=amenity&limit=3`
      )
        .then((response) => response.json())
        .then((result: GeoJSON.Feature[]) => setNeighborhoods(result))
        .catch(onError)
        .catch(onError);
    }
  };
  ...
  const onRemoveItem = (item: GeoJSON.Feature) => {
    setNeighborhood((prev: any) =>
      prev.filter(
        (val: GeoJSON.Feature) =>
          val.properties?.place_id !== item.properties?.place_id
      )
    );
  };
  ...
  const onLocationSelect = (value: GeoJSON.Feature) => {
    init();
    setNeighborhood((prev: any) => [...prev, value]);
  };
  ...
  return (
    <div
      className={classNames(
        className,
        'max-w-[524px] relative h-[400px] top-[174px]'
      )}
    >
      <div className="relative fade-in">
        <div className="flex items-center">
          ...
          {open && neighborhoods && neighborhoods.length > 0 && (
            <div className="geoapify-autocomplete-items !mt-2 z-10">
              {neighborhoods.map((val, i) => (
                <LocationListItem
                  key={'location-item-' + i}
                  item={val}
                  filter={input}
      ...
      {/* Begin Selected Neighbourhood Chips */}
      <div className="my-6 min-h-[116px] max-h-[208px]">
      ...
      {/* End Selected Neighbourhood Chips */}
    </div>
  );
};
